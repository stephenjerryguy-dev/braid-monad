...
await arena.connect(player).riskRoll(true, { value: ethers.parseEther("0.01") });
await entropy.fulfill(await arena.getAddress(), 1n, ethers.zeroPadValue("0x63", 32));
assert.equal(await arena.points(player.address), 25n);
await arena.connect(player).enterRaffle(1);
assert.equal(await arena.entrantCount(1n), 2n);
assert.equal(await arena.points(player.address), 0n);

console.log("Braid contract smoke tests passed: NFT loan, fee staking, entropy game, raffle entry.");
